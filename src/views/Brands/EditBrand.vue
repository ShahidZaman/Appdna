  ...
  <div class="edit-brand">
    <div class="banner">
      <div class="brand-icon-main-container">
        <div v-if="brnadData.iconUrl" class="brand-img-container">
          <img :src="brnadData.iconUrl" alt="" />
        </div>
        <div v-else :style="{color: brnadData.defaultLogo.color, background: brnadData.defaultLogo.background}" class="brand-icon-container">{{capitalizeBrand(brnadData.name)[0]}}</div>
        <div @click="showModal('', 'uploadImg')" class="svg-icon-container">
          <img src="/images/svg/edit-16.svg" alt="edit">
        </div>
      </div>
      <h1>{{ capitalizeBrand(brnadData.name) }}</h1>
    </div>
...
watchEffect(() => {
  brnadData.value = authStore.allTenants.find((item) => item.key === route.params.id);
})
</script>
